...
<div id="headerContainer">
	...
	<div id="userPanel" class="userPanel">
		<div class="userPanelInner">
			...
			<p id="userNote"> 
				{if $this->user->userID != 0}{lang}c4w.header.userNote.user{/lang}{else}{lang}c4w.header.userNote.guest{/lang}{/if}
			</p>
			<div id="userMenu">
				<ul>
					{if $this->user->userID != 0}
						<li id="userMenuLogout"><a href="index.php?action=UserLogout&amp;t={@SECURITY_TOKEN}{@SID_ARG_2ND}"><img src="{icon}logoutS.png{/icon}" alt="" /> <span>{lang}c4w.header.userMenu.logout{/lang}</span></a></li>
						<li id="userMenuProfileEdit"><a href="index.php?form=UserProfileEdit{@SID_ARG_2ND}"><img src="{icon}editS.png{/icon}" alt="" /> <span>{lang}c4w.header.userMenu.profile{/lang}</span></a></li>
						
						{if $additionalUserMenuItems|isset}{@$additionalUserMenuItems}{/if}
						
						{if $this->user->getPermission('admin.general.canUseAcp')}
							<li id="userMenuACP"><a href="acp/index.php?packageID={@PACKAGE_ID}"><img src="{icon}acpS.png{/icon}" alt="" /> <span>{lang}c4w.header.userMenu.acp{/lang}</span></a></li>
						{/if}
					{else}
						<li id="userMenuLogin"><a href="index.php?form=UserLogin{@SID_ARG_2ND}" id="loginButton"><img src="{icon}loginS.png{/icon}" alt="" id="loginButtonImage" /> <span>{lang}c4w.header.userMenu.login{/lang}</span></a></li>
						
						{if !REGISTER_DISABLED}<li id="userMenuRegistration"><a href="index.php?page=Register{@SID_ARG_2ND}"><img src="{icon}registerS.png{/icon}" alt="" /> <span>{lang}c4w.header.userMenu.register{/lang}</span></a></li>{/if}
						
						{if $additionalUserMenuItems|isset}{@$additionalUserMenuItems}{/if}
						
						{if $this->language->countAvailableLanguages() > 1}
							<li id="userMenuLanguage" class="languagePicker"><a id="changeLanguage" class="hidden"><img src="{icon}language{@$this->language->getLanguageCode()|ucfirst}S.png{/icon}" alt="" /> <span>{lang}c4w.header.userMenu.changeLanguage{/lang}</span></a>
	...
	<div id="header">
		 ...
		 * $searchFieldTitle=title of search input field; default=language variable c4w.header.search.query
		...
		{if !$searchFieldTitle|isset}{assign var='searchFieldTitle' value='{lang}c4w.header.search.query{/lang}'}{/if}
		...
		<div id="search">
			<form method="post" action="{@$searchScript}">
				...
				<div class="searchContainer">
					...
					{@SID_INPUT_TAG}
					{if $searchHiddenFields|isset}{@$searchHiddenFields}{/if}
						...
						<div class="searchInputMenu">
							<div class="hidden" id="searchInputMenu">
								<div class="pageMenu smallFont">
									<ul>
										{@$searchFieldOptions}
										{if $searchShowExtendedLink}<li><a href="{@$searchExtendedLink}{if !$searchFieldValue|empty}&amp;defaultQuery={$searchFieldValue|rawurlencode}{/if}">{lang}c4w.header.search.extended{/lang}</a></li>{/if}
									</ul>
								</div>
							</div>
						</div>
					...
					{if $searchShowExtendedLink}
						<noscript>
							<p><a href="{@$searchExtendedLink}">{lang}c4w.header.search.extended{/lang}</a></p>
						</noscript>
					{/if}
				</div>
			</form>
		...
		<div id="logo">
			<div class="logoInner">
				...
				{elseif $this->getStyle()->getVariable('page.logo.image.application.use') == 1}
					<a href="index.php?page=Index{@SID_ARG_2ND}" class="pageLogo">
						<img src="{@RELATIVE_C4W_DIR}images/c4w-header-logo.png" title="{lang}{PAGE_TITLE}{/lang}" alt="" />
					</a>
				{/if}
			</div>
		</div>
