...
<div id="footerContainer">
	<div id="footer">
				...
				<ul>
					...
					{if $stylePickerOptions|count > 1}
						<li class="stylePicker{if !SHOW_CLOCK} last{/if}">
							<a id="changeStyle" class="hidden"><img src="{icon}styleOptionsS.png{/icon}" alt="" /> <span>{lang}c4w.global.changeStyle{/lang}</span></a>
								...
								<form method="get" action="index.php" class="quickJump">
									<div>
										...
										<select name="styleID" onchange="if (this.options[this.selectedIndex].value != 0) this.form.submit()">
											<option value="0">{lang}c4w.global.changeStyle{/lang}</option>
		...
		<p class="copyright">{if 'C4W_BRANDINGFREE'|defined == false}{lang}c4w.global.copyright{/lang}{/if}{if $additionalCopyrightContents|isset}{@$additionalCopyrightContents}{/if}</p>
	</div>
</div>
{if !$this->user->userID && !LOGIN_USE_CAPTCHA}
	<div class="border loginPopup hidden" id="quickLoginBox">
		<form method="post" action="index.php?form=UserLogin" class="container-1">
			...
			<p><label><input tabindex="3" type="checkbox" id="useCookies" name="useCookies" value="1" /> {lang}c4w.header.login.useCookies{/lang}</label></p>
		</form>
	</div>
{/if}