 ...
 * @package	de.community4wcf.c4w.box
 * @subpackage	system.event.listener
 * @category 	Community4WCF (C4W)
 */
class StructuredTemplateBoxListener implements EventListener {
	...
	public function execute($eventObj, $className, $eventName) {
		...
		$html = '<link rel="stylesheet" type="text/css" media="screen" href="'.RELATIVE_C4W_DIR.'style/box.css" />
			<link rel="stylesheet" type="text/css" media="print" href="'.RELATIVE_C4W_DIR.'style/boxPrint.css" />
			<style type="text/css">
				#mainContainer {
					'.C4WCore::getStyle()->getVariable('page.alignment.margin').(C4WCore::getStyle()->getVariable('page.width') !== '' ? 'width:'.C4WCore::getStyle()->getVariable('page.width') : 'max-width:'.C4WCore::getStyle()->getVariable('page.width.max').';min-width:'.C4WCore::getStyle()->getVariable('page.width.min').';').'
				}
			</style>';
